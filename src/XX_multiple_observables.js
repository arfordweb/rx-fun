/*
Sometimes, we may want to respond to values coming from one Observable, but use values from
other observables in the process.  
*/
...
function incrementCapitalLetter(letter) {
  return String.fromCharCode(
    ((letter.charCodeAt(0) - 64) % 26) + 65
  )
}

// First Observable: counts the seconds for 30 seconds
let seconds$ = Observable.create((subscriber) => {
  let secondsSinceStart = 0
  setInterval(
    () => { 
      subscriber.next(secondsSinceStart)
...
// Second Observable: incremenets a letter in a rotation every 2/3 seconds (really, 667ms)
let letters$ = Observable.create((subscriber) => {
  let letter = 'A'
